...
3. **Advanced ELK Stack & Wazuh**: Ingestion pipelines, custom XML decoders, EQL rules.
4. **Detection Engineering & Threat Intel**: Threat modeling, Pyramid of Pain, Sigma rule standards.
5. **Threat Emulation & Red Teaming**: Adversary emulation, Purple Teaming, Atomic Red Team execution.
6. **Microsoft 365 Security Operations**: Unified Audit Logs (UAL), Entra ID identity threats, Exchange Online audits.
7. **Cloud Security for SOC (AWS)**: CloudTrail management, VPC Flow Logs, GuardDuty alerts.
8. **Next-Gen Detection Engineering & AI**: Detection as Code (DaC), AI alert enrichment, Sigmac CLI.
9. **Threat Hunting Fundamentals**: Hypothesis-driven hunting, frequency stacking, parent-child analytics.
10. **Incident Response Life Cycle (NIST/SANS)**: Preparation, triage, containment, eradication, RCA.
11. **Cyber Threat Intelligence (CTI)**: MISP integration, STIX2 models, OpenCTI knowledge graphs.
12. **Advanced Traffic & Network Analysis**: Snort IDS rules, Zeek NSM logs, Wireshark PCAP inspection.
13. **Static Malware Analysis**: Executable triage, PEStudio, MalDoc macros (`olevba`), script deobfuscation.
14. **Wazuh for SOC & GRC Operations**: Regulatory compliance auditing (PCI-DSS/NIST), Osquery SQL.
...
*Copyright © 2026 Zyad Elsheshtawy. All Rights Reserved.*
